// api/[...path].js
// Maneja TODAS las rutas /api/* de forma centralizada.
...
module.exports = async (req, res) => {
  try {
    ...
    // GET /api/quote?uuid=... (público)
    if (url.startsWith('/api/quote') && method === 'GET' && query.uuid) {
    ...
    // GET /api/quotes (listar todas) - debe ir antes que la ruta con uuid
    ...
    if (url.startsWith('/api/search-parts') && method === 'GET') {
    ...
    if (url.startsWith('/api/quote-edit') && method === 'GET') {
      const uuid = query.uuid;
      if (!uuid) {
        res.status(400).json({ error: 'Falta el parámetro uuid' });
        return;
    ...
    if (url.startsWith('/api/quote-edit') && method === 'POST') {
